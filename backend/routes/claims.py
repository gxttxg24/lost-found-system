...
def _paginate(req):
    page = max(_parse_int(req.args.get("page")) or 1, 1)
    page_size = min(max((_parse_int(req.args.get("page_size")) or 10), 1), 100)
...
def get_context():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            users = [{"id": r["id"], "username": r["username"], "role": r.get("role")} for r in cursor.fetchall()]
...
def get_items():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                f"SELECT COUNT(*) AS total FROM items i {where}", tuple(params)
            )
...
@claims_bp.get("/item/<int:item_id>")
@jwt_required()
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if item["user_id"] != current_user_id:
                return fail("只能查看自己物品的认领申请"), 403

            cursor.execute(
                "SELECT COUNT(*) AS total FROM claim_requests WHERE item_id = %s",
                (item_id,),
            )
            total = cursor.fetchone()["total"]
...
def create_claim():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT id, user_id, title, status FROM items WHERE id = %s", (item_id,)
            )
            item = cursor.fetchone()
            if not item:
                return fail("物品不存在"), 404
            if item["status"] == "claimed":
                return fail("该物品已经被认领"), 409
            if item["user_id"] == current_user_id:
                return fail("不能认领自己发布的物品"), 400

            cursor.execute(
                """SELECT id FROM claim_requests
                   WHERE item_id = %s AND applicant_id = %s AND status = 'pending' LIMIT 1""",
                (item_id, current_user_id),
            )
            if cursor.fetchone():
                return fail("你已经提交过该物品的待审核申请"), 409
...
def get_review_claims():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                f"SELECT COUNT(*) AS total FROM claim_requests c {where}", tuple(params)
            )
            total = cursor.fetchone()["total"]
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                """SELECT c.*, i.title AS item_title, i.status AS item_status
                   FROM claim_requests c LEFT JOIN items i ON c.item_id = i.id
                   WHERE c.id = %s""",
                (claim_id,),
            )
            claim = cursor.fetchone()
            if not claim:
                return fail("认领申请不存在"), 404
            if claim["owner_id"] != current_user_id:
                return fail("你没有权限审核该申请"), 403
            if claim["status"] != "pending":
                return fail("该申请已经处理过了"), 400
            ...
            if action == "approve":
                ...
                cursor.execute(
                    "UPDATE items SET status='claimed' WHERE id=%s", (claim["item_id"],)
                )
                cursor.execute(
                    """UPDATE claim_requests SET status='rejected', review_comment=%s
...
def cancel_claim(claim_id):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                """SELECT c.*, i.title AS item_title FROM claim_requests c
                   LEFT JOIN items i ON c.item_id = i.id WHERE c.id = %s""",
                (claim_id,),
            )
            claim = cursor.fetchone()
            if not claim:
            ...
            if claim["applicant_id"] != current_user_id:
                return fail("你只能取消自己的申请"), 403
            if claim["status"] != "pending":
                return fail("只能取消待审核的申请"), 400

            cursor.execute(
                "UPDATE claim_requests SET status='cancelled' WHERE id=%s", (claim_id,)
            )
