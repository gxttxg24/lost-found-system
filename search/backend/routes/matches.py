...
def get_item_matches(item_id):
    """Get smart matches for an item. Auto-computes if none exist. No auth required."""
    ...
    try:
        ...
        return jsonify(fail(f"获取匹配结果失败: {str(e)}")), 500
    finally:
        cursor.close()
        conn.close()
...
def trigger_matching(item_id):
    ...
    try:
        ...
        if item['user_id'] != current_user_id:
            return jsonify(fail("无权操作他人物品")), 403

        results = run_matching(item_id, conn)
        return jsonify(success({'matched_count': len(results)}, "匹配完成"))

    except Exception as e:
        return jsonify(fail(f"匹配失败: {str(e)}")), 500
    finally:
        cursor.close()
        conn.close()
...
def get_my_matches():
    ...
    try:
        ...
        return jsonify(fail(f"获取匹配列表失败: {str(e)}")), 500
    finally:
        cursor.close()
        conn.close()
