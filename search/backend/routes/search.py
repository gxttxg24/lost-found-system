...
def search_items():
    ...
    if item_type not in ('lost', 'found'):
        return jsonify(fail("type 参数必须为 lost 或 found")), 400
    ...
    try:
        ...
        # Application-level TF-IDF re-ranking (only when q provided and sort=relevance)
        ...
        return jsonify(fail(f"搜索失败: {str(e)}")), 500
    finally:
        cursor.close()
        conn.close()
...
def get_categories():
    ...
    try:
        ...
        return jsonify(success(cursor.fetchall()))
    except Exception as e:
        return jsonify(fail(f"获取分类失败: {str(e)}")), 500
    finally:
        cursor.close()
        conn.close()
